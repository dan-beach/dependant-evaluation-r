...
# # Not needed for Reactome. It adds a gene_id column to the dependencies dataframe and about 2 million extra records that are not needed for Reactome
...
source('preprocessing/process_dependencies_23q4.R')
...
# PPI has been pre-processed so comment this line out
#source('preprocessing/process_human_ppi.R')

# Load the processed PPI as interactions
interactions <- read.csv(sprintf('%s/interactions/processed/reactome_base_ppi_v1.csv', data_dir), stringsAsFactors = F)
