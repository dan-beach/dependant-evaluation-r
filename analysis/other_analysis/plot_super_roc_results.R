# Plots ROC AUC values from validation of model trained on the super set (and tested on individual cell lines)
...
for (tag in tags) {
  ...
  for(cell_line_count in counts) {
    x <- read.csv(sprintf('%s/results/super_rocs_full_results_%s_%s.csv', data_dir, tag, cell_line_count))
    print(x$mean_roc[1])
...
write.table(results, sprintf('%s/results/super_roc_by_freq_data.csv', data_dir), row.names=F, sep=',')
...
ggsave(sprintf('%s/results/super_roc_by_cl_freq_sparse50.pdf', data_dir), p)
ggsave(sprintf('%s/results/super_roc_by_cl_freq_sparse50.png', data_dir), p)
