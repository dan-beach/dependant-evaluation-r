...
for (column_name in c('V2', 'V4', 'V5')){
  ...
  for(cell_line in train_cell_lines) {
    print(cell_line)
    row_data <- read.table(sprintf('%s/results/cell_line_results/super_rocs_cls_%s_%s.csv', data_dir, tag, cell_line_count), sep=',', skip=1)  ### %s/results/%s/cls/%s_rocs_cls_%s.csv
    print(head(row_data))
    row_data <-row_data %>%
...
print(full_results)
write.csv(full_results, sprintf('%s/results/super_rocs_full_results_%s_%s.csv', data_dir, tag, cell_line_count))
