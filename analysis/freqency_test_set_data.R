...
if (super_analysis != TRUE) {
  write.table(test_set_info, sprintf('%s/results/cell_line_results/cl_frequency_test_set_data_%s.csv', data_dir, tag), row.names=F, sep=',')
} else {
  write.table(test_set_info, sprintf('%s/results/cell_line_results/super_frequency_test_set_data_%s.csv', data_dir, tag), row.names=F, sep=',')
}
